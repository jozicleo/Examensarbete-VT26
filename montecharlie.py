...
def run_monte_carlo(mc_df, returns_df, n_simulations=10000, seed=42):
    ...
    for sim in range(1, n_simulations + 1):
        ...
        final_wealth_df = build_one_simulation(
            mc_df=mc_df,
            returns_df=returns_df,
            simulation_id=sim,
            rng=sim_rng
        ...
        all_final_wealth.append(final_wealth_df)

        print(f"Klar med Monte Carlo-simulering {sim}/{n_simulations}")

    mc_summary_df = pd.concat(all_final_wealth, ignore_index=True)

    return mc_summary_df
